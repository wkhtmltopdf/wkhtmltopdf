...
EXCLUDE_SRC_TARBALL = [
    ...
    'qt5/qtbase/tests*',
    'qt5/qtbase/util*',
    'qt5/qtbase/lib/fonts*',
    'qt5/qtsvg/dist*',
    'qt5/qtsvg/doc*',
    'qt5/qtsvg/examples*',
    'qt5/qtsvg/tests*',
    'qt5/qtxmlpatterns/dist*',
    'qt5/qtxmlpatterns/examples*',
    'qt5/qtxmlpatterns/tests*',
    'qt5/qtwebkit/*ChangeLog*',
    'qt5/qtwebkit/Source/*.vcxproj',
    'qt5/qtwebkit/Source/*.xcodeproj'
    'qt5/qtwebkit/Source/JavaScriptCore/icu*',
    'qt5/qtwebkit/Source/JavaScriptCore/tests*',
    'qt5/qtwebkit/Source/WebCore/icu*',
    'qt5/qtwebkit/Source/WTF/icu*',
    'qt5/qtwebkit/Tools/CygwinDownloader*',
    'qt5/qtwebkit/Tools/DumpRenderTree*',
    'qt5/qtwebkit/Tools/iExploder*',
    'qt5/qtwebkit/Tools/Scripts*',
    'qt5/qtwebkit/Tools/TestWebKitAPI*',
    'qt5/qtwebkit/Tools/WebKitTestRunner*'
]
...
def get_version(basedir):
    mkdir_p(basedir)
    text = PROJECT_SETUP['version'] #open(os.path.join(basedir, '..', 'VERSION'), 'r').read()
    if '-' not in text:
        return (text, text)
...
def check_source_tarball(config):
    ...
    for module in QT_SUBMODULES:
        if not exists(os.path.join(os.getcwd(), 'qt5', module, '.git')):
...
def build_source_tarball(config, basedir):
    ...
    for module in QT_SUBMODULES:
        os.chdir(os.path.join(root_dir, 'qt5', module))
        shell('git clean -fdx')
...
def build_mingw64_cross(config, basedir):
    ...
    os.environ['SQLITE3SRCDIR'] = '%s/../qt5/qtbase/src/3rdparty/sqlite' % basedir

    mkdir_p(qtdir)
    build_qtmodule(qtdir, 'qtbase', 'make -j%d' % CPU_COUNT,
        '%s/../qt5/qtbase/configure %s' % (basedir, configure_args))
    build_qtmodule(qtdir, 'qtsvg',  'make -j%d' % CPU_COUNT,
        '%s/qtbase/bin/qmake %s/../qt5/qtsvg/qtsvg.pro' % (qtdir, basedir))
    build_qtmodule(qtdir, 'qtxmlpatterns', 'make -j%d' % CPU_COUNT,
        '%s/qtbase/bin/qmake %s/../qt5/qtxmlpatterns/qtxmlpatterns.pro' % (qtdir, basedir))
    build_qtmodule(qtdir, 'qtwebkit', 'make -j%d' % CPU_COUNT,
        '%s/qtbase/bin/qmake %s/../qt5/qtwebkit/WebKit.pro WEBKIT_CONFIG-=build_webkit2' % (qtdir, basedir))
...
def main():
    ...
    if not exists(os.path.join(rootdir, 'qt5', 'qtbase', 'configure')):
