...
QString ReflectClass::get(const char * name) {
    ...
    if (!elms.contains(QString::fromUtf8(name, i))) return QString();
    return elms[QString::fromUtf8(name,i)]->get(name + (name[i] == '.'?i+1:i));
}

bool ReflectClass::set(const char * name, const QString & value) {
	int i=0;
	while (name[i] !=0 && name[i] != '.' && name[i] != '[') ++i;
    if (!elms.contains(QString::fromUtf8(name, i))) return false;
    return elms[QString::fromUtf8(name,i)]->set(name + (name[i] == '.'?i+1:i), value);
}
